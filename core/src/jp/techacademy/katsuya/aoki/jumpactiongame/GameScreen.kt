package jp.techacademy.katsuya.aoki.jumpactiongame
...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    ...
    var sound_gameover = Gdx.audio.newSound(Gdx.files.internal("gongend.mp3"))
    var sound_start = Gdx.audio.newSound(Gdx.files.internal("gong.mp3"))
    ...
    private fun checkGameOver() {
        if (mHeightSoFar - CAMERA_HEIGHT / 2 > mPlayer.y || flag == 1) {
            sound_gameover.play(1.0f)
            sound_gameover.dispose();
